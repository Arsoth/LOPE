...
### [x] 1. Require a primary click before saving a profile

- [x] Define how the primary-click output is recognized across presets, raw
      records, and imported JSON.
- [x] Block every mouse-write path when the selected profile has no assigned
      primary click, including button-only and combined saves.
- [x] Show an actionable validation message and identify the profile that needs
      a primary click assignment.
- [x] Add regression coverage proving rejected saves do not invoke the write
...
### [x] 2. Make onboard DPI dragging smooth and responsive

- [x] Reproduce the current delay, stutter, or handle-position mismatch while
      dragging DPI stages on supported mice.
- [x] Trace pointer events, value snapping, stage reordering, and SwiftUI view
      updates to identify the source of lag or dropped intermediate positions.
- [x] Update the interaction path so the dragged handle tracks continuously
      without blocking work or applying redundant updates.
- [x] Preserve supported-value snapping, strict stage ordering, default/shift
      assignments, accessibility behavior, and text editing.
- [x] Add regression coverage for continuous drag updates and stage crossings,
...
### [x] 3. Add onboard RGB profile editing for supported G-series mice

- [x] Determine which G-series mice and onboard profile formats expose writable
      RGB zones, and document the capability and zone names in the device
      descriptors.
- [x] Define a safe read/write representation for per-profile RGB zone colors,
      including validation for unsupported or read-only devices.
- [x] Show an RGB settings row only for profiles and devices that advertise the
      capability, with a color swatch and zone name for each zone.
- [x] Open a color-wheel picker when a zone is clicked and apply one color to
      all zones when the row is Shift-clicked.
- [x] Include RGB changes in save, backup, import/export, and read-back
      verification flows where the profile format supports them.
- [x] Add focused tests for capability gating, color conversion, per-zone
...
### [x] 4. Organize backups by mouse model and keep JSON in Documents

- [x] Store backups in a subdirectory named for the compatible mouse model.
- [x] Keep the mouse model in every backup filename as a secondary sanity check.
- [x] Default JSON import and export panels to the user's Documents folder
      instead of the backup directory.
- [x] Preserve device matching, backup discovery, and restore behavior across
