...
struct ProfileOutputParserSelfTest {
  static func main() {
    guard AppearancePreference.allCases == [.system, .light, .dark],
      AppearancePreference.light.label == "Light"
    else {
    ...
    for (index, testCase) in cases.enumerated() {
      ...
      guard actual == testCase.1 else {
        fatalError(
          "profile capacity parser case \(index + 1) returned \(String(describing: actual)); expected \(String(describing: testCase.1))"
        )
      }
    }
    ...
    guard ProfileOutputParser.scrollWheelOutputLabel("90 10 00 00") == "Scroll down",
      ...
      !ProfileOutputParser.isScrollWheelOutput("FFFFFFFF")
    else {
    ...
    guard withoutAccess.last?.isWiredAccessPrompt == true,
      ...
      !DeviceClassification.isMXSeriesMouse(name: "G603", productID: "0xB01C")
    else {
    ...
    guard g603Profile.refreshGuidance?.sleepDescription.contains("3 seconds") == true,
      ...
      OnboardProfileRefreshPolicy.maximumPollAttempts == 60
    else {
    ...
    for (index, testCase) in selectionCases.enumerated() {
      ...
      guard actual == testCase.3 else {
        fatalError(
          "profile selection case \(index + 1) returned \(String(describing: actual)); expected \(String(describing: testCase.3))"
        )
      }
    }
    ...
    guard rangeCapabilities.minimum == 400,
      ...
      rangeCapabilities.snappedValue(for: 1573) == 1550
    else {
    ...
    guard listCapabilities.supportedValues == [800, 1600, 3200],
      ...
      listCapabilities.snappedValue(for: 1800, upperBound: 1599) == 800
    else {
    ...
    guard extendedPolling.supportedRates == [125, 1000, 8000],
      extendedPolling.currentRate == 1000,
      extendedPolling.accepts(8000),
      !extendedPolling.accepts(500)
    else {
      fatalError("extended polling-rate output parsing failed")
    }
    ...
    guard legacyPolling.supportedRates == [125, 250, 333, 1000],
      legacyPolling.currentRate == 333
    else {
      fatalError("legacy polling-rate output parsing failed")
    }
    ...
    do {
      ...
      guard discovered == expectedBackups,
        BackupStorage.mouseIdentifier(fromBackupFilename: "profile-1-save-20260912-120000.logiob")
          == nil,
        BackupStorage.restoreArguments(for: first) == ["restore", first.path, "--yes"]
      else {
    ...
    guard parsedRGBLine?.index == 1,
      ...
      ProfileOutputParser.rgbZone(from: "RGB zone 0: AABBCC (mode 0x01)") == nil
    else {
    ...
    guard
      ...
      g502.rgbCapabilities(deviceName: "G502 HERO", productID: "0xC08B", profileFormat: 7) == nil
    else {
    ...
    guard
      unsupported.rgbCapabilities(
        deviceName: "G603 LIGHTSPEED", productID: "0xB01C", profileFormat: 5) == nil
    else {
      fatalError("unsupported device incorrectly advertised RGB")
    }

    print(
      "profile metadata self-test: capacity, selection, DPI, backup-storage, and RGB cases passed")
  }
}
