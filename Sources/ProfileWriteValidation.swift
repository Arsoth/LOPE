...
enum ProfileWriteValidation {
    ...
    static func inaccessibleGShiftPrimaryClickMessage(
        ...
        guard gShiftButtonRaws.contains(where: isPrimaryClick),
              !normalButtonRaws.contains(where: isGShiftBinding) else { return nil }
        ...
        return profileLabel + " has primary click assigned only on the G-Shift layer, but no Normal-layer button activates G-Shift. Assign G-Shift to a Normal button or add a primary click to the Normal layer, then save again."
    }
