...
private struct StatusEvent: Identifiable {
  let id = UUID()
  let message: String
  let timestamp: Date
...
extension View {
  fileprivate func pointerCursor(enabled: Bool = true) -> some View {
...
struct ContentView: View {
  ...
  var body: some View {
    ...
    .onChange(of: model.status) { newStatus in
      statusHistory.insert(StatusEvent(message: newStatus), at: 0)
      if statusHistory.count > 10 {
        statusHistory.removeLast(statusHistory.count - 10)
      }
    }
