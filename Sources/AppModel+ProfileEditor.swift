...
extension AppModel {
    ...
    func resetProfileEditorDraft() {
        ...
        if hasSpecificMouseProfile {
            ...
            let rawByNumber = Dictionary(uniqueKeysWithValues: normalButtonRows.map { ($0.id, $0.currentRaw) })
            var scrollLabels: [String: String] = [:]
            for number in profileEditorButtonNumbers {
                if let raw = rawByNumber[number], let label = ProfileOutputParser.scrollWheelOutputLabel(raw) {
        ...
        profileEditorName = hasSpecificMouseProfile
            ? base.name
        ...
        for number in profileEditorButtonNumbers {
            names[number] = base.button(for: number)?.control
    ...
    private func buildProfileEditorDescriptor() -> MouseProfileDescriptor? {
        ...
        guard !trimmedID.isEmpty, !profileEditorButtonNumbers.isEmpty, let base = profileEditorBase else {
            return nil
        }
        ...
        let buttons = profileEditorButtonNumbers.map { number -> MouseProfileDescriptor.Button in
            let trimmedControl = (profileEditorButtonNames[number] ?? "").trimmingCharacters(in: .whitespaces)
            let existing = base.button(for: number)
            return MouseProfileDescriptor.Button(
                number: number,
                control: trimmedControl.isEmpty ? (existing?.control ?? "Button \(number)") : trimmedControl,
                aliases: existing?.aliases ?? [],
                notes: existing?.notes
            )
        }
        let sources = profileEditorSources
    ...
    func saveProfileEditorDraft() -> URL? {
        guard !isMXSeriesMouse else {
            status = "LOPE does not save profiles for MX-series mice; their controls are managed by Logi Options+, not onboard memory."
            return nil
        }
    ...
    func importProfileEditorDraft() {
        ...
        guard panel.runModal() == .OK, let url = panel.url,
              ...
              let descriptor = try? JSONDecoder().decode(MouseProfileDescriptor.self, from: data) else {
            return
        }
        ...
        for number in profileEditorButtonNumbers {
            profileEditorButtonNames[number] = descriptor.button(for: number)?.control
