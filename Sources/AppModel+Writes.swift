...
extension AppModel {
    ...
    func setConfigurationDirectory(_ url: URL) {
        ...
        UserDefaults.standard.set(url.path, forKey: "\(AppConstants.defaultsPrefix).configurationDirectory")
    ...
    func createGeneratedProfile() -> URL? {
        guard !busy, !loadingProfile, !profiles.isEmpty, !hasSpecificMouseProfile,
              !isMXSeriesMouse else { return nil }
        ...
        let baseIdentifier = "auto-" + BackupStorage.sanitizedMouseIdentifier(
        ...
        let dpiRange: MouseProfileDescriptor.DPIRange? = {
            guard let minimum = dpiCapabilities.minimum, let maximum = dpiCapabilities.maximum else { return nil }
        ...
        guard let data = try? encoder.encode(descriptor), (try? data.write(to: url, options: .atomic)) != nil else {
            status = "Could not write the generated profile to \(directory.path)."
            return nil
        }

        MouseProfileCatalog.reload(customProfilesDirectory: customProfilesDirectory)
        status = "Created \(url.lastPathComponent) with generic button names. Rename its controls in the custom profiles folder any time."
        refresh()
        return url
    }
